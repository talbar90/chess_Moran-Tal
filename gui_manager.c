...
void spManagerDraw(SPGuiManager* src) {
	...
	else if (src->activeWin == SP_MODE_ACTIVE){
		spModeWindowDraw(src->chooseModeWin);
	}
	else if (src->activeWin == SP_DIFF_ACTIVE){
		spDiffWindowDraw(src->chooseDiffWin);
	}
	else if (src->activeWin == SP_COLOR_ACTIVE){
		spColorWindowDraw(src->chooseColorWin);
	}
...
SP_MANAGER_EVENT spManagerHandleEvent(SPGuiManager* src, SDL_Event* event) {
	...
	if (src->activeWin == SP_MAIN_WINDOW_ACTIVE) {
		...
		SP_MODE_EVENT modeEvent = spModeWindowHandleEvent(src->chooseModeWin, event);
		return handleManagerDueToModeEvent(src, modeEvent);
	}
	else if (src->activeWin == SP_DIFF_ACTIVE) {
		SP_DIFF_EVENT diffEvent = spDiffWindowHandleEvent(src->chooseDiffWin, event);
		return handleManagerDueToDiffEvent(src, diffEvent);
	}
	else if (src->activeWin == SP_COLOR_ACTIVE) {
		SP_COLOR_EVENT colorEvent = spColorWindowHandleEvent(src->chooseColorWin, event);
		return handleManagerDueToDiffEvent(src, colorEvent);
	}
//	else {
...
SP_MANAGER_EVENT handleManagerDueToMainEvent(SPGuiManager* src,
	...
	if (event == SP_MAIN_START) {
		...
		src->chooseModeWin = spModeWindowCreate();
		src->activeWin = SP_MODE_ACTIVE;
//		spModeWindowShow(src->chooseModeWin);
	}
	if (event == SP_MAIN_LOAD) {
		//TODO: src->load = spLoadWindowCreate();
		spMainWindowHide(src->mainWin);
		src->activeWin = SP_LOAD_ACTIVE;
...
SP_MANAGER_EVENT handleManagerDueToModeEvent(SPGuiManager* src,
	...
	if (event == SP_MODE_DIFF) {
		spModeWindowHide(src->chooseModeWin);
		src->activeWin = SP_DIFF_ACTIVE;
		spDiffWindowShow(src->chooseDiffWin);
	}
	if (event == SP_MODE_START) {
		spModeWindowHide(src->chooseModeWin);
...
//		spGameWindowShow(src->gameWin);
	...
	if (event == SP_MODE_BACK) {
		spModeWindowHide(src->mainWin);
		src->activeWin = SP_MAIN_WINDOW_ACTIVE;
		spMainWindowShow(src->mainWin);

	}
	if (event == SP_MODE_QUIT) {
		return SP_MANAGER_QUTT;
	}
	return SP_MANAGER_NONE;
}
SP_MANAGER_EVENT handleManagerDueToDiffEvent(SPGuiManager* src,
	...
	if (event == SP_DIFF_SET) {
		spDiffWindowHide(src->chooseDiffWin);
		src->activeWin = SP_COLOR_ACTIVE;
		spColorWindowShow(src->chooseColorWin);
	}
	else if (event == SP_DIFF_BACK) {
		spDiffWindowHide(src->chooseDiffWin);
		src->activeWin = SP_MODE_ACTIVE;
		spModeWindowShow(src->chooseModeWin);
	}
...
SP_MANAGER_EVENT handleManagerDueToColorEvent(SPGuiManager* src,
	...
	if (event == SP_COLOR_START) {
		spColorWindowHide(src->chooseColorWin);
		src->activeWin = SP_GAME_WINDOW_ACTIVE;
//		spGameWindowShow(src->chooseDiffWin);
	}
	if (event == SP_COLOR_BACK) {
		spColorWindowHide(src->chooseDiffWin);
		src->activeWin = SP_MODE_ACTIVE;
		spDiffWindowShow(src->chooseDiffWin);
	}
	if (event == SP_COLOR_QUIT) {
		return SP_MANAGER_QUTT;
	}
	return SP_MANAGER_NONE;
}
