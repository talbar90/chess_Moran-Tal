...
SPMainWin* spMainWindowCreate() {
	...
	res->load_game = SDL_CreateTextureFromSurface(res->mainRenderer, loadingSurface);
	if (res->load_game == NULL ) {
	...
	res->new_game = SDL_CreateTextureFromSurface(res->mainRenderer, loadingSurface);
	if (res->new_game == NULL ) {
		SDL_FreeSurface(loadingSurface);
		spMainWindowDestroy(res);
	...
	res->quit = SDL_CreateTextureFromSurface(res->mainRenderer, loadingSurface);
	if (res->quit == NULL ) {
		SDL_FreeSurface(loadingSurface);
		spMainWindowDestroy(res);
		printf("couldn't create start.bmp texture\n");
...
void spMainWindowDestroy(SPMainWin* src) {
	...
	if (src->load_game != NULL ) {
		SDL_DestroyTexture(src->load_game);
	}
	if (src->new_game != NULL ) {
		SDL_DestroyTexture(src->new_game);
	}
	if (src->quit != NULL ) {
		SDL_DestroyTexture(src->quit);
	}
...
void spMainWindowDraw(SPMainWin* src) {
	...
	SDL_Rect new_game = { .x = 75, .y = 100, .h = 100, .w = 250 };
	SDL_Rect load_game = { .x = 75, .y = 250, .h = 100, .w = 250 };
	SDL_Rect quit = { .x = 75, .y = 400, .h = 100, .w = 250 };
	SDL_SetRenderDrawColor(src->mainRenderer, 255, 255, 255, 255);
	SDL_RenderClear(src->mainRenderer);
	SDL_RenderCopy(src->mainRenderer, src->new_game, NULL, &new_game);
	SDL_RenderCopy(src->mainRenderer, src->load_game, NULL, &load_game);
	SDL_RenderCopy(src->mainRenderer, src->quit, NULL, &quit);
...
SP_MAIN_EVENT spMainWindowHandleEvent(SPMainWin* src, SDL_Event* event) {
	...
	case SDL_MOUSEBUTTONUP:
		if (isClickOnNewGame(event->button.x, event->button.y)) {
			...
			return SP_MAIN_LOAD;
		} else if (isClickOnLoadQuit(event->button.x, event->button.y)) {
			return SP_MAIN_QUIT;
		}
		break;
...
//}
//
//// attach pic to game piece and vice-versa
...
//}
//
//// helper func to avoid compilation error regarding casting from char to int
//int char2int(char c){
...
//}
//
//// find button node in a specific UI tree
...
//			Label *lbl = (Label*)tile_node->children[0]->control;
//			if (!is_players_piece(curr_player, get_piece_by_pic(lbl->name))) return;
